...
add_filter('gform_pre_render_22', 'populate_html_22');

function populate_html_22( $form ) {
	if ( empty( $form['id'] ) ) {
			return $form;
	}
			...
			?>
				...
				<script>
					...
					jQuery(window).load(function() {
						jQuery('#gform_next_button_22_52').hide();
					});
					...
					cardForm.on('FORM_RESPONSE', (response) => {
						...
						if(token != null) {
							jQuery('#input_22_54').val(token);
							jQuery('#gform_next_button_22_52').show();
						}
...
add_action( 'gform_pre_submission_22', 'pre_submission_22' );
...
function pre_submission_22( $form ) {
		
	$subtotal = rgpost( 'input_32' );
	$token = rgpost( 'input_54' );
	
	$total_value = floor($subtotal * 100);
	
	$_POST['input_55'] = $token; // This needs to correlate to the field that gets the token
	$_POST['input_56'] = $total_value; // This needs to correlate to the field that gets the total amount/ value
	
	// Field holding the response
	$response_field = 'input_57';
