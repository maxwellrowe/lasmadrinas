...
add_filter('gform_pre_render_24', 'populate_html_24');

function populate_html_24( $form ) {
	if ( empty( $form['id'] ) ) {
			return $form;
	}
			...
			?>
				...
				<script>
					...
					jQuery(window).load(function() {
						jQuery('#gform_next_button_24_74').hide();
					});
					...
					cardForm.on('FORM_RESPONSE', (response) => {
						...
						if(token != null) {
							jQuery('#input_24_76').val(token);
							jQuery('#gform_next_button_24_74').show();
						}
...
add_action( 'gform_pre_submission_24', 'pre_submission_24' );
...
function pre_submission_24( $form ) {
		
	$subtotal = rgpost( 'input_70' );
	$token = rgpost( 'input_76' );
	
	$total_value = floor($subtotal * 100);
	
	$_POST['input_77'] = $token; // This needs to correlate to the field that gets the token
	$_POST['input_78'] = $total_value; // This needs to correlate to the field that gets the total amount/ value
	
	// Field holding the response
	$response_field = 'input_79';
