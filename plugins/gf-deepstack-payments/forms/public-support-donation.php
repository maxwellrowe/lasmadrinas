...
add_filter('gform_pre_render_23', 'populate_html_23');

function populate_html_23( $form ) {
	if ( empty( $form['id'] ) ) {
			return $form;
	}
			...
			?>
				...
				<script>
					...
					jQuery(window).load(function() {
						jQuery('#gform_next_button_23_45').hide();
					});
					...
					cardForm.on('FORM_RESPONSE', (response) => {
						...
						if(token != null) {
							jQuery('#input_23_47').val(token);
							jQuery('#gform_next_button_23_45').show();
						}
...
add_action( 'gform_pre_submission_23', 'pre_submission_23' );
...
function pre_submission_23( $form ) {
		
	$subtotal = rgpost( 'input_32' );
	$token = rgpost( 'input_47' );
	
	$total_value = floor($subtotal * 100);
	
	$_POST['input_48'] = $token; // This needs to correlate to the field that gets the token
	$_POST['input_49'] = $total_value; // This needs to correlate to the field that gets the total amount/ value
	
	// Field holding the response
	$response_field = 'input_50';
