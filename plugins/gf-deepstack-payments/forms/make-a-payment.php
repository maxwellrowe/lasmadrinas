...
add_filter('gform_pre_render_16', 'populate_html');
			...
			?>
				...
				<script>
					...
					jQuery(window).load(function() {
						jQuery('#gform_next_button_16_62').hide();
					});
					...
					cardForm.on('FORM_RESPONSE', (response) => {
						...
						if(token != null) {
							jQuery('#input_16_61').val(token);
							jQuery('#gform_next_button_16_62').show();
						}
...
add_action( 'gform_pre_submission_16', 'pre_submission' );
...
function pre_submission( $form ) {
	...
	$token = rgpost( 'input_61' );
	
	$total_value = floor($subtotal * 100);
	
	$_POST['input_63'] = $token; // This needs to correlate to the field that gets the token
	$_POST['input_64'] = $total_value; // This needs to correlate to the field that gets the total amount/ value
	
	// Field holding the response
	$response_field = 'input_65';
