...
add_filter('gform_pre_render_19', 'populate_html_19');

function populate_html_19( $form ) {
	if ( empty( $form['id'] ) ) {
			return $form;
	}
			...
			?>
				...
				<script>
					...
					jQuery(window).load(function() {
						jQuery('#gform_next_button_19_55').hide();
					});
					...
					cardForm.on('FORM_RESPONSE', (response) => {
						...
						if(token != null) {
							jQuery('#input_19_57').val(token);
							jQuery('#gform_next_button_19_55').show();
						}
...
add_action( 'gform_pre_submission_19', 'pre_submission_19' );
...
function pre_submission_19( $form ) {
		
	$subtotal = rgpost( 'input_32' );
	$token = rgpost( 'input_57' );
	
	$total_value = floor($subtotal * 100);
	
	$_POST['input_58'] = $token; // This needs to correlate to the field that gets the token
	$_POST['input_59'] = $total_value; // This needs to correlate to the field that gets the total amount/ value
	
	// Field holding the response
	$response_field = 'input_60';
