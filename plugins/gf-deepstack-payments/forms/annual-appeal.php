...
add_filter('gform_pre_render_17', 'populate_html_17');

function populate_html_17( $form ) {
	if ( empty( $form['id'] ) ) {
			return $form;
	}
			...
			?>
				...
				<script>
					...
					jQuery(window).load(function() {
						jQuery('#gform_next_button_17_42').hide();
					});
					...
					cardForm.on('FORM_RESPONSE', (response) => {
						...
						if(token != null) {
							jQuery('#input_17_44').val(token);
							jQuery('#gform_next_button_17_42').show();
						}
...
add_action( 'gform_pre_submission_17', 'pre_submission_17' );
...
function pre_submission_17( $form ) {
		
	$subtotal = rgpost( 'input_32' );
	$token = rgpost( 'input_44' );
	
	$total_value = floor($subtotal * 100);
	
	$_POST['input_45'] = $token; // This needs to correlate to the field that gets the token
	$_POST['input_46'] = $total_value; // This needs to correlate to the field that gets the total amount/ value
	
	// Field holding the response
	$response_field = 'input_47';
