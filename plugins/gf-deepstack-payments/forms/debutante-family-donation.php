...
add_filter('gform_pre_render_20', 'populate_html_20');

function populate_html_20( $form ) {
	if ( empty( $form['id'] ) ) {
			return $form;
	}
			...
			?>
				...
				<script>
					...
					jQuery(window).load(function() {
						jQuery('#gform_next_button_20_45').hide();
					});
					...
					cardForm.on('FORM_RESPONSE', (response) => {
						...
						if(token != null) {
							jQuery('#input_20_47').val(token);
							jQuery('#gform_next_button_20_45').show();
						}
...
add_action( 'gform_pre_submission_20', 'pre_submission_20' );
...
function pre_submission_20( $form ) {
