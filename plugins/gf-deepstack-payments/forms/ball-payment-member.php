...
add_filter('gform_pre_render_18', 'populate_html_18');

function populate_html_18( $form ) {
	if ( empty( $form['id'] ) ) {
			return $form;
	}
			...
			?>
				...
				<script>
					...
					jQuery(window).load(function() {
						jQuery('#gform_next_button_18_72').hide();
					});
					...
					cardForm.on('FORM_RESPONSE', (response) => {
						...
						if(token != null) {
							jQuery('#input_18_74').val(token);
							jQuery('#gform_next_button_18_72').show();
						}
...
add_action( 'gform_pre_submission_18', 'pre_submission_18' );
...
function pre_submission_18( $form ) {
		
	$subtotal = rgpost( 'input_69' );
	$token = rgpost( 'input_74' );
	
	$total_value = floor($subtotal * 100);
	
	$_POST['input_75'] = $token; // This needs to correlate to the field that gets the token
	$_POST['input_76'] = $total_value; // This needs to correlate to the field that gets the total amount/ value
	
	// Field holding the response
	$response_field = 'input_77';
